...
**若 Skill 来自外部链接**（检测 A 发现）：
- 询问用户：「检测到 Skill 来自外部链接，需要保存到标准路径 `skills/ablesci-checkin/SKILL.md`，是否保存？」
- 用户回答"是" → 保存并告知完成
- 用户回答"否" → 不保存，继续执行后续流程
...
若检测 A 发现 Skill 来自外部链接（Gist/GitHub 等）：
- 询问用户：「检测到 Skill 来自外部链接，需要保存到标准路径 `skills/ablesci-checkin/SKILL.md`，是否保存？」
- 用户回答"是" → 保存到标准路径，告知完成，继续执行情况 A-2
- 用户回答"否" → 不保存，继续执行情况 A-2
...
>
> **首次使用前**：使用「配置 AbleSci 签到」触发引导流程，AI 自动创建目录和文件。详见上方「首次配置」章节。
>
> 工作区根目录通常位于：
...
设置 AbleSci
```
AI 自动读取本 Skill，检测 Skill 位置和配置文件状态，自动引导完成配置（新建/查看/新增/修改/删除）。详见上方「首次配置」章节。
